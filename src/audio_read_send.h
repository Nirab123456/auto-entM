...
class AUDIO_RS
{
    private:
        ...
        std::shared_ptr <std::atomic<uint64_t>> Abs_Idx_{nullptr};


        uint32_t* i2s_word_buffer_ptr_ = nullptr;
        size_t buffer_len_ = 0;

        void Ring_Clear_Rst();
    public:
        AUDIO_RS();
        explicit AUDIO_RS(uint32_t* i2s_word_buffer, size_t length);
        AUDIO_RS(
        ...
        //accessor
        uint32_t* i2s_word_buffer() const;
